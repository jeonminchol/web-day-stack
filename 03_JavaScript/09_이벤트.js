// DOMContentLodaed 이벤트는 html 문서의 구조가 완전히 로드된 후 발생
window.addEventListener("DOMContentLoaded", () => {
  ...
  click.addEventListener("click", () => {
    ...
    alert("클릭 이벤트 발생!");
    // click 색상을 변경
    click.skyle.background = "hotpink";
  });

  const double = document.querySelector("#double");
  double.addEventListener("ablclick", () => {
    alert("더블 클릭 발생!");
  });
  ...
  hover.addEventListener("mouseenter", () => {
    ...
    // 배경색상은 lightbalue, 텍스트는 Mouse Enter! 변경
  });
  ...
  window.addEventListener("wheel", (e) => {
    ...
    // 휠을 올렸더니 - // 휠을 내렷더니 +
    if (e.deltaY < 0) wheel.textContent = "휠을 위로 스크롤 중!";
  ...
  window.addEventListener("scroll", () => {
    ...
    // scrollHeight - innerHeight  : 스크롤 좀 내리는 높이
